...
const EditDomain = ({ domain }: { domain: Domain }) => {
  ...
  const { data, setData, put, processing, errors, reset } = useForm({
    ...
    asking_price: domain.asking_price  ,
    for_sale: domain.for_sale  ,
  ...
  const submit = (e: React.FormEvent) => {
    e.preventDefault();
    put(route('domains.update', {id: domain.id}));
  };
  ...
  return (
    <div className="container mx-auto py-8">
      <h1 className="text-2xl font-bold mb-6 max-w-xl mx-auto">Edit Domain: {domain.name}</h1>
          ...
          {/* <Button
            ...
            onClick={() => {
              if (confirm('Are you sure you want to delete this domain?')) {
                route.delete(route('domains.destroy', domain.id));
              }
            }}
            disabled={processing}
          >
